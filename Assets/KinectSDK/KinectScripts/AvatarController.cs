...
//		}
    ...
    protected void MoveAvatar(uint UserID)
    {
        ...
        // Posición del cuerpo (Kinect space)
        Vector3 trans = kinectManager.GetUserPosition(UserID);

        // Calibración inicial de offsets (se mantiene igual)
        if (!offsetCalibrated)
        {
            ...
            // Colocar relativo a la cámara SOLO si está activado
        ...
        // --- TRASLACIÓN DEL ROOT ---
        // Solo actualizamos la posición local del bodyRoot si queremos que el AvatarController
        // gobierne la traslación (modo "relativo al sensor").
        if (offsetRelativeToSensor)
        {
        ...
        // else: no tocamos bodyRoot.localPosition -> la traslación la hace tu CharacterController
    }
